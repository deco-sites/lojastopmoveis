...
export default function NotFound({ termDescription = "Resultados de busca encontrados para:", title = "Ops!", hints, searchTerm, }: SectionProps<typeof loader>) {
    return (<div class="w-full">
      <div class="flex flex-col gap-[10px] lg:flex-row lg:justify-around lg:mt-[30px] lg:mb-[60px] items-center">
        ...
        {hints && (<div class="bg-[#F9BE00] p-5 flex flex-col gap-[20px] text-primary rounded-[20px] lg:justify-center lg:p-[60px] h-fit max-lg:mb-[30px]">
            <h6 class="font-bold lg:text-xl">{hints.title}</h6>
            <ul class="flex flex-col gap-3 list-disc list-inside">
              {hints.hints.map((hint) => (<li class="text-xs lg:text-sm font-medium items-center gap-2 list-item">
                  <span>{hint}</span>
                </li>))}
            </ul>
          </div>)}
