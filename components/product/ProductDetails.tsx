...
function ProductInfo({ page, shipmentPolitics, shareableNetworks }: {
    ...
    const { description, productID, offers, name, isVariantOf, isSimilarTo, url, } = product;
    const { price = 0, listPrice, seller, availability, installment } = useOffer(offers);
    ...
    return (<>
      ...
      {availability === "https://schema.org/InStock"
            ...
            <div class="flex flex-col items-start gap-2 ">
              {listPrice !== price && (<span class="line-through text-base-300 text-sm">
            ...
            <div class="flex flex-col">
              <span class="text-secondary text-2xl font-bold">
                ou {installment?.billingDuration}x de {formatPrice(installment?.billingIncrement, offers!.priceCurrency)}
              </span>
            </div>
            <div class="flex items-center gap-[10px] py-[10px]">
              ...
              {discount && forPrice && (<span class="font-bold max-lg:text-[10px] max-lg:px-[5px] text-[12px] border border-[#4A4B51] rounded-md text-[#4A4B51] py-[2px] tracking-[2px] px-[10px] ">
      ...
      <details className="collapse collapse-plus mt-[30px]">
        ...
        <div className="readmore !flex-col text-xs px-0 pl-[30px] mt-3 leading-tight collapse-content text-base-300">
          ...
          <p className="readmore-content whitespace-break-spaces !line-clamp-none" dangerouslySetInnerHTML={{ __html: description ? description : "" }}>
          </p>
        </div>
      </details>
      ...
      {shareableNetworks && (<div class="flex items-center gap-5 my-5">
          ...
          <ul class="gap-2 flex items-center justify-between">
            {shareableNetworks.map((network) => (<li class="bg-secondary w-8 h-8 rounded-full group transition-all">
                <a href={getShareLink({
                    network,
                    productName: isVariantOf?.name ?? name ?? "",
                    url: url ?? "",
                })} target="_blank" rel="noopener noreferrer" class="flex items-center justify-center w-full h-full group-hover:text-white text-primary">
...
function Details({ page, variant, shipmentPolitics, shareableNetworks, highlights, device, }: {
    page: ProductDetailsPage;
    variant: Variant;
    shipmentPolitics?: Props["shipmentPolitics"];
    shareableNetworks?: Props["shareableNetworks"];
    ...
    if (variant === "slider") {
        ...
        <div id={id} class="flex flex-col lg:flex-row gap-4 lg:justify-center">
          {/* Product Images */}
          <ProductDetailsImages images={product.image!} width={WIDTH} height={HEIGHT} aspect={ASPECT_RATIO} product={product} highlights={highlights} device={device}/>
...
function ProductDetails({ page, variant: maybeVar = "auto", shipmentPolitics, shareableNetworks, highlights, device }: SectionProps<ReturnType<typeof loader>>) {
    /**
    ...
    return (<div class="py-0">
      {page
            ? (<Details page={page} variant={variant} shipmentPolitics={shipmentPolitics} shareableNetworks={shareableNetworks} highlights={highlights} device={device}/>)
            : <NotFound />}
    </div>);
}
