...
export function loader(props: Props, _req: Request, ctx: AppContext) {
    if (!props.productShelf) return { success: false, ...props };
...
const TimedProductShelf = (props: SectionProps<typeof loader>)=> {
    ...
    const { 
        ...
        if (!productShelf || isCurrentDateTime) return null;

    return (
        <div style={{ backgroundColor: frameColor }} className="rounded-lg md:rounded-none">
            <div className="container w-full m-auto py-[30px] px-[24px]">
                <div className="flex flex-col items-center justify-center mb-[15px] md:flex-row md:justify-between">
                    ...
                    <CountdownTimer 
                        ...
                        frameColor={countdownFrameColor} 
                        timeDate={countdownTimerDate} 
                        timeHour={countdownTimerHours}
                        numberColor={countdownNumberColor}
                    />
                </div>
                <LocalProductShelf 
                    products={productShelf} 
                    cardLayout={productShelfConfig.cardLayout} 
                    highlights={productShelfConfig.highlights}
                    layout={productShelfConfig.layout}
                    showPaginationArrows={productShelfConfig.showPaginationArrows} 
