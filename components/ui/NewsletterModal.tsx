...
const subscribe = Runtime.create("deco-sites/std/actions/vtex/newsletter/subscribe.ts");
...
function InputNewsletter({ name, placeholder, required, type }: InputNewletterProps) {
    return (<input name={name} type={type} class="input lg:h-12 h-9 px-5 join-item w-full mb-2.5 first:mt-5 border-2 border-[#C5C6CB] rounded-full placeholder:text-placeholder !outline-none lg:text-base text-xs" placeholder={placeholder} required={required}/>);
}
function NewsletterModal({ isOpen, form, text, modalSignExpiredDate, modalCloseExpiredDate, }: Props & {
    isOpen: boolean;
}) {
      ...
      <dialog ref={modalRef} class="modal bg-primary-content bg-opacity-70">
        <form method="dialog" class="modal-box overflow-visible p-10">
          ...
          {success.value
            ...
            : (<>
                <Icon class="mx-auto mb-5 block" id="Logo" height={70} width={94.6}/>
        ...
        <form method="dialog" className="modal-backdrop bg-primary bg-opacity-40">
          <button aria-label="fechar modal" onClick={() => setCookieOnCloseModal("closed", modalCloseExpiredDate)}>
            fechar
          </button>
        </form>
      </dialog>
